...
function freshBattle(enemyIndex = 0) {
  ...
  state.battle = { turn: 1, energy: state.run.relics.includes('sunzi') ? 4 : 3, player: { name: hero.name, hp: Math.min(state.run.hp ?? maxHp, maxHp), maxHp, block: 0, rage: 0, mystery: 0, played: 0, evade: false }, enemy: { ...enemyData, maxHp: enemyData.hp, block: 0, burn: 0 }, drawPile: shuffle(deck), hand: [], discard: [], logs: [], enemyIndex, relicTriggered: false };
  draw(state.battle, 5);
...
function hit(s, raw) { const card = cards[state.selectedCard]; const weaponBonus = state.run.relics.includes('qinggang') && card?.type === 'attack' && !s.relicTriggered ? 10 : 0; if (weaponBonus) { s.relicTriggered = true; log('青釭剑：本场第一次武技伤害 +10。'); } const bonus = state.heroId === 'zhangfei' ? s.player.rage * 0.05 : 0; const damage = Math.round((raw + weaponBonus) * (1 + bonus)); const blocked = Math.min(s.enemy.block, damage); s.enemy.block -= blocked; s.enemy.hp = Math.max(0, s.enemy.hp - (damage - blocked)); log(`${card?.name || '攻击'}造成 ${damage - blocked} 伤害${blocked ? `（被护甲抵消 ${blocked}）` : ''}${upgraded(state.selectedCard) ? '（强化）' : ''}。`); }
...
function playCard(index) { const s = state.battle; const id = s.hand[index]; const card = cards[id]; if (!card || s.energy < card.cost || s.enemy.hp <= 0) return; state.selectedCard = id; s.energy -= card.cost; s.hand.splice(index, 1); s.discard.push(id); card.effect(s); s.player.played++; if (state.heroId === 'zhaoyun' && s.player.played % 3 === 0) { hit(s, 40); log('龙胆连击：追加 40 伤害。'); } if (s.enemy.hp <= 0) { state.reward = { type: s.enemyIndex === 1 ? 'relic' : 'card' }; } render(); }
