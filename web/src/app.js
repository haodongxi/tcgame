...
function normalizeBattle(battle) { if (!battle) return battle; if (!battle.enemies) battle.enemies = [battle.enemy]; battle.player ||= {}; battle.player.bleed ||= 0; battle.player.weak ||= 0; battle.player.vulnerable ||= false; battle.enemies.forEach(enemy => { enemy.attackBonus ||= 0; enemy.defeatHandled ||= false; }); battle.targetIndex = Math.max(0, Math.min(battle.targetIndex ?? 0, battle.enemies.length - 1)); battle.enemy = battle.enemies[battle.targetIndex] || battle.enemies[0]; return battle; }
...
function freshBattle(enemyIndex = 0) {
  ...
  state.battle = { turn: 1, chapter: state.chapter || 0, energy: state.run.relics.includes('sunzi') ? 4 : 3, player: { name: hero.name, hp: Math.min(state.run.hp ?? maxHp, maxHp), maxHp, block: 0, rage: 0, rageCap: 10, rageMultiplier: 0.05, mystery: 0, dragon: 0, played: 0, attackCardsPlayed: 0, evade: false, weak: 0, vulnerable: false, bleed: 0 }, enemies, targetIndex: 0, enemy: enemies[0], drawPile: shuffle(deck), hand: [], discard: [], logs: [], enemyIndex, relicTriggered: false };
  draw(state.battle, 5);
...
function enemyPanelView(s) { return `<section id="enemy-panel" class="enemy-group" aria-label="敌方单位">${s.enemies.map((enemy,index)=>`<button class="enemy-bar ${enemy.hp <= 0 ? 'defeated' : ''} ${index === s.targetIndex ? 'selected-target' : ''}" data-target-index="${index}" ${enemy.hp <= 0 ? 'disabled' : ''}><div class="avatar">${s.enemyIndex===2?'将':'敌'}</div><div><div class="enemy-name">${enemy.name}</div><div class="hp-text">兵力 ${enemy.hp}/${enemy.maxHp}</div><div class="bar"><span style="width:${Math.max(0,enemy.hp/enemy.maxHp*100)}%"></span></div>${enemyStatusView(enemy)}</div><div class="intent">${enemy.hp > 0 ? (enemy.boss && s.chapter === 0 && s.turn % 3 === 0 ? '黄天符法 · 70伤害' : enemy.boss && s.chapter === 1 && s.turn % 2 === 0 ? '斩击 · 破甲' : enemy.boss && s.chapter === 2 && s.turn % 4 === 0 && enemy.hp > enemy.maxHp * .3 ? '召唤死士' : s.turn % 2 ? `攻击 ${enemyAttack(s, enemy)}` : `护甲 +${enemy.blockGain || 20}`) : '已击破'}</div></button>`).join('')}</section>`; }
