...
function cardPreviewView() { if (state.selectedIndex === null || !state.selectedCard) return ''; const card = cards[state.selectedCard]; if (!card) return ''; return `<div class="card-preview-copy"><b>${card.name}</b><span>消耗 ${card.cost} 气力 · ${cardNeedsEnemy(card) ? '敌方目标' : '立即生效'}</span><small>${card.desc}</small></div>${cardNeedsEnemy(card) ? '<small class="card-preview-hint">请选择敌方目标使用</small>' : '<button class="secondary card-use" data-action="use-card">使用</button>'}`; }
