...
function continueRun() { const save = loadRunSave(); if (!save) return; save.run.path ||= []; state = { ...state, screen: save.screen, heroId: save.heroId, chapter: save.chapter || 0, node: save.node, run: save.run, battle: normalizeBattle(save.battle), selectedCard: save.selectedCard, selectedIndex: null, reward: save.reward }; render(); }
...
function cloneEnemy(enemy) { return { ...enemy, maxHp: enemy.hp, block: 0, burn: 0, berserk: false, stunned: 0, bossPhase: false }; }
...
function enemyAttack(s, enemy = s.enemy) { const eliteBonus = s.enemyIndex === 1 ? Math.floor((s.turn - 1) / 3) * 20 : 0; const bossBonus = s.enemyIndex === 2 && enemy.hp <= enemy.maxHp * 0.3 ? 2 : s.enemyIndex === 2 && enemy.hp <= enemy.maxHp * 0.5 ? 1.2 : 1; return Math.round((enemy.attack + eliteBonus) * bossBonus); }
...
function enemyAct(s) { const alive = (s.enemies || [s.enemy]).filter(enemy => enemy.hp > 0); for (const enemy of alive) { s.enemy = enemy; if (enemy.burn) { enemy.hp = Math.max(0, enemy.hp - enemy.burn * 12); log(`${enemy.name}受到灼烧 ${enemy.burn * 12} 伤害。`); enemy.burn--; } if (enemy.stunned > 0) { log(`${enemy.name}被眩晕，无法行动。`); enemy.stunned--; continue; } if (enemy.hp <= 0) continue; if (bossSpecialAct(s, enemy)) continue; if (s.enemyIndex === 1 && s.turn > 1 && s.turn % 3 === 0) log(`${enemy.name}：蓄势完成。`); if (s.enemyIndex === 2 && !enemy.berserk && enemy.hp <= enemy.maxHp * 0.5) { enemy.berserk = true; log(`${enemy.name}进入暴走。`); } if (s.turn % 2) enemyStrike(s, enemy, enemyAttack(s, enemy)); else { enemy.block += enemy.blockGain || 20; log(`${enemy.name}获得 ${enemy.blockGain || 20} 护甲。`); } } if (s.player.weak > 0) s.player.weak--; s.player.rageOnHit = false; s.player.reflectDamage = 0; s.player.reflectRatio = 0; s.player.vulnerable = false; syncTarget(s); }
...
function mapView() { const chapter = state.chapter || 0; const routes = chapterRouteColumns[chapter] || routeColumns; const options = routes[state.node] || []; const start = Math.max(0, Math.min(state.node - 2, routes.length - 5)); const visible = routes.slice(start, start + 5); const path = (state.run?.path || []).filter(item => item.chapter === chapter); const recentPath = path.slice(-5).map(item => `${item.node + 1}层·${routes[item.node]?.find(option => option.id === item.type)?.label || item.type}`).join('　›　'); return shell(`${topbar()}<section class="map-card"><h2>${chapterRoutes[chapter].name}</h2><p class="map-summary">主将：${heroes[state.heroId].name}　·　章节 ${chapter + 1}/3　·　行军进度 ${state.node}/${routes.length - 1}　·　主题：${chapterRoutes[chapter].theme}</p><div class="map-line">${visible.map((column,i)=>{const index=start+i;const labels=column.map(option=>`<span class="node-type ${option.id}">${option.mark} ${option.label}</span>`).join('');return `<div class="node ${index<state.node?'done':''} ${index===state.node?'active':''}"><button disabled>${index<state.node?'✓':index===state.node?'⚔':'·'}</button><div class="node-types">${labels}</div></div>`;}).join('')}</div>${recentPath ? `<p class="path-review"><b>最近行军：</b>${recentPath}</p>` : ''}<h3 class="route-heading">选择下一处行军节点</h3><div class="route-options">${options.map(option=>`<button class="route-option" data-node-type="${option.id}"><span class="route-mark">${option.mark}</span><span><b>${option.label}</b><small>${option.desc}</small></span><span>›</span></button>`).join('')}</div></section>`); }
...
function playerStatusView(player) { return `<div class="status-row">${statusChip('虚弱', player.weak, 'control')}${player.vulnerable ? '<span class="status-chip danger">易伤</span>' : ''}${player.reflectDamage ? '<span class="status-chip">反伤</span>' : ''}${player.reflectRatio ? '<span class="status-chip">反弹 30%</span>' : ''}</div>`; }
...
function bind() {
  ...
  document.querySelector('[data-action="confirm-hero"]')?.addEventListener('click',()=>{state.run={deck:defaultDeck(),gold:80,hp:maxHpForHero(),relics:[],souls:[],upgraded:{},path:[]};state.meta.stats.runs++; saveMeta(); state.screen='map';render();});
