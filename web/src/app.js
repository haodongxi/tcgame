...
let state = { screen: 'title', heroId: null, node: 0, meta: loadMeta(), run: null, battle: null, selectedCard: null, reward: null, busy: false, animateDeal: false };
...
function resolvePlayCard(index) { const s = state.battle; const id = s.hand[index]; const card = cards[id]; if (!card || s.energy < card.cost || s.enemy.hp <= 0) { state.busy = false; refreshBattleParts(); return; } state.selectedCard = id; s.energy -= card.cost; s.hand.splice(index, 1); s.discard.push(id); if (state.heroId === 'zhugeliang' && card.strategy) { s.player.mystery = Math.min(mysteryCap(), s.player.mystery + 1); log(`神机：计策牌触发，当前 ${s.player.mystery}/${mysteryCap()} 层。`); } card.effect(s); s.player.played++; if (state.heroId === 'zhaoyun') { s.player.dragon = (s.player.dragon + 1) % 3; if (s.player.dragon === 0) { hit(s, 40); log('龙胆连击：追加 40 伤害。'); } } if (s.enemy.hp <= 0) { state.reward = { type: s.enemyIndex === 1 ? 'relic' : 'card' }; } state.busy = false; state.animateDeal = false; state.reward ? render() : refreshBattleParts(); }
...
function refreshBattleParts() {
  ...
  ['enemy-panel', 'battle-log', 'player-panel', 'hand-panel'].forEach(id => {
    document.getElementById(id)?.replaceWith(next.querySelector(`#${id}`));
  });
...
function cardView(id, index, reward=false) { const c=cards[id]; const mark = upgraded(id) ? ' · 强化' : ''; const dealClass = !reward && state.animateDeal ? 'deal-card' : ''; return `<button class="card ${c.type} ${dealClass} ${upgraded(id)?'enhanced':''} ${state.selectedCard===id&&!reward?'selected':''}" style="--deal-index:${index}" data-card="${id}" data-index="${index}"><span class="cost">${c.cost}</span><div class="card-art">${c.art}</div><strong>${c.name}${mark}</strong><small>${descriptionFor(id)}</small></button>`; }
...
function battleView() { const s=state.battle, p=s.player, e=s.enemy; const mechanism = state.heroId==='zhangfei' ? `狂怒 ${p.rage}层` : state.heroId==='zhugeliang' ? `神机 ${p.mystery}/${mysteryCap()}层` : `龙胆 ${p.dragon}/3`; return shell(`<div class="battle-screen">${topbar()}<section id="enemy-panel" class="enemy-bar"><div class="avatar">${s.enemyIndex===2?'董':'敌'}</div><div><div class="enemy-name">${e.name}${e.berserk?' · 暴走':''}</div><div class="hp-text">兵力 ${e.hp}/${e.maxHp}</div><div class="bar"><span style="width:${Math.max(0,e.hp/e.maxHp*100)}%"></span></div></div><div class="intent">下回合<br><b>${intentText(s)}</b></div></section><section id="battle-log" class="log-panel"><h2>战斗日志</h2>${s.logs.slice(-8).map(x=>`<div class="log-line">${x}</div>`).join('')}</section><section id="player-panel" class="player-panel"><div class="avatar">${heroes[state.heroId].mark}</div><div><div class="player-heading"><b>${p.name}</b><span>兵力 ${p.hp}/${p.maxHp}</span></div><div class="bar"><span style="width:${Math.max(0,p.hp/p.maxHp*100)}%"></span></div><div class="stats"><span class="stat">护甲 <b>${p.block}</b></span><span class="stat">气力 <b>${s.energy}/3</b></span><span class="stat">${mechanism}</span></div>${activeBonds().length?`<div class="relic-row"><span class="bond-chip">羁绊：${activeBonds().join('、')}</span></div>`:''}${state.run.souls.length?`<div class="relic-row">${soulView()}</div>`:''}${state.run.relics.length?`<div class="relic-row">${relicView()}</div>`:''}</div></section><section id="hand-panel" class="hand-area"><div class="hand-title"><span>‹ 手牌 ›</span><span>${s.hand.length} 张</span></div><div class="hand">${s.hand.map((id,i)=>cardView(id,i)).join('')}</div><div class="battle-actions"><span class="turn">第 ${s.turn} 回合 · 点击卡牌查看并打出</span><button class="primary" data-action="end" ${e.hp<=0?'disabled':''}>结束回合</button></div></section></div>${state.reward?rewardModal():''}`); }
