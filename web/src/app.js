...
function cardView(id, index, reward=false) { const c=cards[id]; const mark = upgraded(id) ? ' · 强化' : ''; const dealClass = !reward && state.animateDeal ? 'deal-card' : ''; return `<button class="card ${c.type} ${dealClass} ${upgraded(id)?'enhanced':''} ${state.selectedCard===id&&!reward?'selected':''}" aria-label="${c.name}" style="--deal-index:${index}" data-card="${id}" data-index="${index}"><span class="cost">${c.cost}</span><div class="card-art">${c.art}</div><strong>${c.name}${mark}</strong></button>`; }
