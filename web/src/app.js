...
const cards = {
  ...
  frenzySlash: { cardId: 'ZHANGFEI_009', name: '狂刃连斩', cost: 1, type: 'attack', rarity: 'rare', art: '斩', image: './assets/cards/zhangfei/9.png', desc: '消耗 2 层狂怒，连续攻击 3 次，每次 25 伤害', effect: s => { s.player.rage = Math.max(0, s.player.rage - 2); for (let i = 0; i < 3; i++) if (s.enemy.hp > 0) hit(s, 25); } },
  ...
  counterwall: { cardId: 'ZHANGFEI_012', name: '坚壁反戈', cost: 2, type: 'skill', rarity: 'rare', art: '壁', image: './assets/cards/zhangfei/12.png', desc: '消耗 3 狂怒，获得 40 护甲，本回合受击反弹 30 伤害', effect: s => { const spent = Math.min(3, s.player.rage); s.player.rage -= spent; s.player.block += 40; s.player.reflect = true; log(`坚壁反戈：获得 40 护甲，消耗 ${spent} 狂怒。`); } },
  bloodRoar: { cardId: 'ZHANGFEI_013', name: '嗜血咆哮', cost: 2, type: 'attack', rarity: 'rare', art: '嗜', image: './assets/cards/zhangfei/13.png', desc: '造成 80 伤害并按伤害吸血', effect: s => { const before = s.enemy.hp; hit(s, 80); const heal = Math.round((before - s.enemy.hp) * (0.2 + s.player.rage * 0.02)); s.player.hp = Math.min(s.player.maxHp, s.player.hp + heal); log(`嗜血咆哮：回复 ${heal} 兵力。`); } },
  ...
  onePass: { cardId: 'ZHANGFEI_017', name: '一夫当关', cost: 2, type: 'skill', rarity: 'legendary', art: '关', image: './assets/cards/zhangfei/17.png', desc: '消耗全部狂怒，每层获得 12 护甲，本回合反弹伤害', effect: s => { const armor = s.player.rage * 12; s.player.rage = 0; s.player.block += armor; s.player.reflect = true; log(`一夫当关：获得 ${armor} 护甲。`); } },
...
function hit(s, raw) { const card = cards[state.selectedCard]; const weaponBonus = state.run.relics.includes('qinggang') && card?.type === 'attack' && !s.relicTriggered ? 10 : 0; if (weaponBonus) { s.relicTriggered = true; log('青釭剑：本场第一次武技伤害 +10。'); } const rageBonus = state.heroId === 'zhangfei' ? s.player.rage * (s.player.rageMultiplier || 0.05) : 0; const lowHealthBonus = state.heroId === 'zhangfei' && s.player.bloodRush && s.player.hp < s.player.maxHp * 0.5 ? 0.10 : 0; const strategyBonus = state.heroId === 'zhugeliang' && card?.strategy ? s.player.mystery * 0.04 : 0; const soulBonus = soulDamageBonus(card); const damage = Math.round((raw + weaponBonus) * (1 + rageBonus + lowHealthBonus + strategyBonus + soulBonus)); const blocked = Math.min(s.enemy.block, damage); s.enemy.block -= blocked; s.enemy.hp = Math.max(0, s.enemy.hp - (damage - blocked)); if (strategyBonus) log(`神机：计策伤害 +${Math.round(strategyBonus * 100)}%。`); if (soulBonus) log(`将魂/羁绊：伤害 +${Math.round(soulBonus * 100)}%。`); log(`${card?.name || '攻击'}造成 ${damage - blocked} 伤害${blocked ? `（被护甲抵消 ${blocked}）` : ''}${upgraded(state.selectedCard) ? '（强化）' : ''}。`); }
...
function bind() {
  ...
  document.querySelector('[data-target="enemy"]')?.addEventListener('click',event=>{event.stopPropagation();confirmTarget();});
  ...
  app.onclick = event => { if (state.selectedIndex !== null && !event.target.closest('.card, [data-target="enemy"]')) cancelSelection(); };
