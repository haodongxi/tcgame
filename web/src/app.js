...
const cards = {
  ...
  formation: { name: '八阵图', cost: 1, type: 'skill', strategy: true, art: '阵', desc: '获得 30 护甲，神机 +2', effect: s => { s.player.block += 30; s.player.mystery = Math.min(8, s.player.mystery + 2); log('八阵图：神机 +2。'); } },
...
function maxHpForHero() { return heroes[state.heroId].maxHp + (state.meta.talents?.sharpBlade || 0) * 50; }
...
function freshBattle(enemyIndex = 0) {
  ...
  const maxHp = hero.maxHp + (state.meta.talents?.sharpBlade || 0) * 50;
...
function hit(s, raw) { const card = cards[state.selectedCard]; const weaponBonus = state.run.relics.includes('qinggang') && card?.type === 'attack' && !s.relicTriggered ? 10 : 0; if (weaponBonus) { s.relicTriggered = true; log('青釭剑：本场第一次武技伤害 +10。'); } const rageBonus = state.heroId === 'zhangfei' ? s.player.rage * 0.05 : 0; const strategyBonus = state.heroId === 'zhugeliang' && card?.strategy ? s.player.mystery * 0.04 : 0; const damage = Math.round((raw + weaponBonus) * (1 + rageBonus + strategyBonus)); const blocked = Math.min(s.enemy.block, damage); s.enemy.block -= blocked; s.enemy.hp = Math.max(0, s.enemy.hp - (damage - blocked)); if (strategyBonus) log(`神机：计策伤害 +${Math.round(strategyBonus * 100)}%。`); log(`${card?.name || '攻击'}造成 ${damage - blocked} 伤害${blocked ? `（被护甲抵消 ${blocked}）` : ''}${upgraded(state.selectedCard) ? '（强化）' : ''}。`); }
...
function cardView(id, index, reward=false) { const c=cards[id]; const mark = upgraded(id) ? ' · 强化' : ''; return `<button class="card ${c.type} ${upgraded(id)?'enhanced':''} ${state.selectedCard===id&&!reward?'selected':''}" data-card="${id}" data-index="${index}"><span class="cost">${c.cost}</span><div class="card-art">${c.art}</div><strong>${c.name}${mark}</strong><small>${descriptionFor(id)}</small></button>`; }
...
function bind() {
  ...
  document.querySelector('[data-action="confirm-hero"]')?.addEventListener('click',()=>{state.run={deck:defaultDeck(),gold:80,hp:maxHpForHero(),relics:[],upgraded:{}};state.screen='map';render();});
