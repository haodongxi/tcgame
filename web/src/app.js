...
function enemyAct(s) { const alive = (s.enemies || [s.enemy]).filter(enemy => enemy.hp > 0); for (const enemy of alive) { s.enemy = enemy; if (enemy.burn) { enemy.hp = Math.max(0, enemy.hp - enemy.burn * 12); log(`${enemy.name}受到灼烧 ${enemy.burn * 12} 伤害。`); enemy.burn--; } if (s.enemyIndex === 1 && s.turn > 1 && s.turn % 3 === 0) log(`${enemy.name}：蓄势完成。`); if (s.enemyIndex === 2 && !enemy.berserk && enemy.hp <= enemy.maxHp * 0.5) { enemy.berserk = true; log(`${enemy.name}进入暴走。`); } if (enemy.hp <= 0) continue; if (s.turn % 2) { if (s.player.evade) { log('单骑救主抵消了本次攻击。'); s.player.evade = false; } else { const attack = enemyAttack(s, enemy); const blocked = Math.min(s.player.block, attack); s.player.block -= blocked; const damage = attack - blocked; s.player.hp -= damage; log(`${enemy.name}攻击，造成 ${damage} 伤害。`); if (state.heroId === 'zhangfei' && damage > 0) { addRage(s, s.player.hp < s.player.maxHp * 0.5 ? 2 : 1); if (s.player.rageOnHit) addRage(s, 1); } } } else { enemy.block += enemy.blockGain || 20; log(`${enemy.name}获得 ${enemy.blockGain || 20} 护甲。`); } } syncTarget(s); }
