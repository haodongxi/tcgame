...
const cards = {
  spear: { cardId: 'BASE_ATTACK_001', name: '挺矛直刺', cost: 1, type: 'attack', rarity: 'basic', art: '⚔', image: './public/assets/cards/base/1.webp', desc: '造成 40 点武技伤害', effect: s => hit(s, 40) },
  guard: { cardId: 'BASE_DEFEND_001', name: '列阵防御', cost: 1, type: 'skill', rarity: 'basic', art: '🛡', image: './public/assets/cards/base/2.webp', desc: '获得 30 点护甲', effect: s => { s.player.block += 30; log('列阵防御：获得 30 护甲。'); } },
  quick: { cardId: 'BASE_COMMON_001', name: '速攻', cost: 0, type: 'attack', rarity: 'common', art: '⚔', image: './public/assets/cards/base/3.webp', desc: '造成 20 点伤害，抽 1 张牌', effect: s => { hit(s, 20); draw(s, 1); log('速攻：抽 1 张牌。'); } },
  roar: { cardId: 'ZHANGFEI_001', name: '咆哮突进', cost: 1, type: 'attack', rarity: 'basic', art: '怒', image: './public/assets/cards/zhangfei/1.webp', desc: '造成 60 伤害，获得 2 层狂怒', effect: s => { hit(s, 60); addRage(s, 2); log('咆哮突进：狂怒 +2。'); } },
  bridge: { cardId: 'ZHANGFEI_002', name: '据水断桥', cost: 2, type: 'skill', rarity: 'basic', art: '桥', image: './public/assets/cards/zhangfei/2.webp', desc: '获得 50 护甲；消耗 3 层狂怒，每层额外 +10 护甲', effect: s => { const spent = Math.min(3, s.player.rage); s.player.rage -= spent; s.player.block += 50 + spent * 10; log(`据水断桥：获得 ${50 + spent * 10} 护甲。`); } },
  yandang: { cardId: 'ZHANGFEI_003', name: '燕人咆哮', cost: 3, type: 'attack', rarity: 'basic', art: '吼', image: './public/assets/cards/zhangfei/3.webp', desc: '造成 220 伤害，消耗全部狂怒，每层 +12 伤害', effect: s => { const damage = 220 + s.player.rage * 12; hit(s, damage); log(`燕人咆哮：消耗 ${s.player.rage} 层狂怒。`); s.player.rage = 0; } },
  naked: { cardId: 'ZHANGFEI_004', name: '裸衣死战', cost: 1, type: 'attack', rarity: 'common', art: '血', image: './public/assets/cards/zhangfei/4.webp', desc: '失去 50 最大兵力，获得 5 层狂怒，抽 1 张牌', effect: s => { s.player.maxHp = Math.max(1, s.player.maxHp - 50); s.player.hp = Math.min(s.player.hp, s.player.maxHp); addRage(s, 5); draw(s, 1); log('裸衣死战：最大兵力 -50，抽 1 张牌。'); } },
  anger: { cardId: 'ZHANGFEI_005', name: '怒发冲冠', cost: 1, type: 'attack', rarity: 'common', art: '怒', image: './public/assets/cards/zhangfei/5.webp', desc: '获得 3 层狂怒，本回合受到伤害 +10%', effect: s => { addRage(s, 3); s.player.vulnerable = true; log('怒发冲冠：狂怒 +3，本回合承伤提高。'); } },
  bloodDebt: { cardId: 'ZHANGFEI_006', name: '血债血偿', cost: 2, type: 'power', rarity: 'common', art: '血', image: './public/assets/cards/zhangfei/6.webp', desc: '消耗全部狂怒，每层回复 8 兵力', effect: s => { const heal = s.player.rage * 8; s.player.rage = 0; s.player.hp = Math.min(s.player.maxHp, s.player.hp + heal); log(`血债血偿：回复 ${heal} 兵力。`); } },
  peerless: { cardId: 'ZHANGFEI_007', name: '万夫莫当', cost: 2, type: 'attack', rarity: 'rare', art: '将', image: './public/assets/cards/zhangfei/7.webp', desc: '造成 80 伤害；狂怒≥5时附带破甲', effect: s => { hit(s, 80); if (s.player.rage >= 5) s.enemy.armorBreak = 2; log('万夫莫当：满足狂怒条件时附带破甲。'); } },
  earthSmash: { cardId: 'ZHANGFEI_008', name: '裂地猛击', cost: 2, type: 'attack', rarity: 'rare', art: '地', image: './public/assets/cards/zhangfei/8.webp', desc: '造成 90 伤害；狂怒≥7时眩晕 1 回合', effect: s => { hit(s, 90); if (s.player.rage >= 7) s.enemy.stunned = 1; log('裂地猛击：满足狂怒条件时眩晕敌人。'); } },
  frenzySlash: { cardId: 'ZHANGFEI_009', name: '狂刃连斩', cost: 1, type: 'attack', rarity: 'rare', art: '斩', image: './public/assets/cards/zhangfei/9.webp', desc: '消耗 2 层狂怒，连续攻击 3 次，每次 25 伤害', effect: s => { s.player.rage = Math.max(0, s.player.rage - 2); const strikes = s.player.thousandBlades ? 5 : 3; for (let i = 0; i < strikes; i++) if (s.enemy.hp > 0) hit(s, 25); log(`狂刃连斩：攻击 ${strikes} 段。`); } },
  fangFinisher: { cardId: 'ZHANGFEI_010', name: '裂牙补刀', cost: 0, type: 'attack', rarity: 'rare', art: '牙', image: './public/assets/cards/zhangfei/10.webp', desc: '消耗 1 层狂怒，追加 1 次攻击', effect: s => { if (s.player.rage > 0) { s.player.rage--; hit(s, 35); } else log('裂牙补刀：没有狂怒，无法追加攻击。'); } },
  shield: { cardId: 'ZHANGFEI_011', name: '立盾拒敌', cost: 1, type: 'skill', rarity: 'rare', art: '盾', image: './public/assets/cards/zhangfei/11.webp', desc: '获得 25 护甲；本回合受击时额外叠加狂怒', effect: s => { s.player.block += 25; s.player.rageOnHit = true; log('立盾拒敌：获得 25 护甲。'); } },
  counterwall: { cardId: 'ZHANGFEI_012', name: '坚壁反戈', cost: 2, type: 'skill', rarity: 'rare', art: '壁', image: './public/assets/cards/zhangfei/12.webp', desc: '消耗 3 狂怒，获得 40 护甲，本回合受击反弹 30 伤害', effect: s => { const spent = Math.min(3, s.player.rage); s.player.rage -= spent; s.player.block += 40; s.player.reflectDamage = 30; log(`坚壁反戈：获得 40 护甲，消耗 ${spent} 狂怒。`); } },
  bloodRoar: { cardId: 'ZHANGFEI_013', name: '嗜血咆哮', cost: 2, type: 'attack', rarity: 'rare', art: '嗜', image: './public/assets/cards/zhangfei/13.webp', desc: '造成 80 伤害并按伤害吸血', effect: s => { const dealt = hit(s, 80); const heal = Math.round(dealt * (0.2 + s.player.rage * 0.02)); s.player.hp = Math.min(s.player.maxHp, s.player.hp + heal); log(`嗜血咆哮：回复 ${heal} 兵力。`); } },
  bloodRush: { cardId: 'ZHANGFEI_014', name: '狂血奔涌', cost: 1, type: 'power', rarity: 'rare', art: '奔', image: './public/assets/cards/zhangfei/14.webp', desc: '本局兵力低于 50% 时，武技伤害 +10%', effect: s => { s.player.bloodRush = true; log('狂血奔涌：低兵力时获得额外武技伤害。'); } },
  lineage: { cardId: 'ZHANGFEI_015', name: '燕人血脉', cost: 2, type: 'power', rarity: 'legendary', art: '脉', image: './public/assets/cards/zhangfei/15.webp', desc: '本局狂怒上限 15，每层伤害加成提升至 6%', effect: s => { s.player.rageCap = 15; s.player.rageMultiplier = 0.06; log('燕人血脉：狂怒上限提升至 15。'); } },
  thousandBlades: { cardId: 'ZHANGFEI_016', name: '千刃狂潮', cost: 2, type: 'power', rarity: 'legendary', art: '刃', image: './public/assets/cards/zhangfei/16.webp', desc: '本局狂刃连斩攻击段数 +2', effect: s => { s.player.thousandBlades = true; log('千刃狂潮：狂刃连斩强化。'); } },
  onePass: { cardId: 'ZHANGFEI_017', name: '一夫当关', cost: 2, type: 'skill', rarity: 'legendary', art: '关', image: './public/assets/cards/zhangfei/17.webp', desc: '消耗全部狂怒，每层获得 12 护甲，本回合反弹 30% 伤害', effect: s => { const armor = s.player.rage * 12; s.player.rage = 0; s.player.reflectRatio = 0.30; s.player.block += armor; log(`一夫当关：获得 ${armor} 护甲，本回合反弹 30% 伤害。`); } },
  berserk: { cardId: 'ZHANGFEI_018', name: '浴血疯魔', cost: 2, type: 'power', rarity: 'legendary', art: '魔', image: './public/assets/cards/zhangfei/18.webp', desc: '本局兵力低于 50% 时，所有攻击 100% 吸血', effect: s => { s.player.berserk = true; log('浴血疯魔：低兵力时攻击获得强力吸血。'); } },
...
function createSeed() { return (Math.floor(Math.random() * 0x100000000) >>> 0) || 1; }
function normalizeBattle(battle) { if (!battle) return battle; if (!battle.enemies) battle.enemies = [battle.enemy]; battle.player ||= {}; battle.player.bleed ||= 0; battle.player.weak ||= 0; battle.player.vulnerable ||= false; battle.enemies.forEach(enemy => { enemy.attackBonus ||= 0; enemy.defeatHandled ||= false; }); const hasTarget = Number.isInteger(battle.targetIndex) && battle.targetIndex >= 0 && battle.targetIndex < battle.enemies.length; battle.targetIndex = hasTarget ? battle.targetIndex : null; battle.enemy = battle.targetIndex === null ? battle.enemies.find(enemy => enemy.hp > 0) || battle.enemies[0] : battle.enemies[battle.targetIndex]; return battle; }
function continueRun() { const save = loadRunSave(); if (!save) return; save.run.path ||= []; save.run.seed ||= createSeed(); save.run.rngState ||= save.run.seed; state = { ...state, screen: save.screen, heroId: save.heroId, chapter: save.chapter || 0, node: save.node, run: save.run, battle: normalizeBattle(save.battle), selectedCard: save.selectedCard, selectedIndex: null, reward: save.reward }; render(); }
...
const enemyGroups = {
  normal: [
    ...
    [{ name: '山贼头目', hp: 150, attack: 35, charge: 60 }, { name: '山贼喽啰', hp: 50, attack: 20 }, { name: '山贼喽啰', hp: 50, attack: 20 }],
...
const chapterEnemyGroups = [
  ...
  {
    normal: [
      [{ name: '西凉枪兵', hp: 120, attack: 30, armorBreak: 5 }, { name: '西凉枪兵', hp: 120, attack: 30, armorBreak: 5 }, { name: '西凉枪兵', hp: 120, attack: 30, armorBreak: 5 }],
      [{ name: '西凉重盾手', hp: 200, attack: 25, blockGain: 25 }, { name: '西凉重盾手', hp: 200, attack: 25, blockGain: 25 }],
      [{ name: '飞熊军斥候', hp: 100, attack: 18, multiAttack: 2 }, { name: '飞熊军斥候', hp: 100, attack: 18, multiAttack: 2 }, { name: '西凉弩手', hp: 80, attack: 35 }],
      [{ name: '西凉监军', hp: 180, attack: 20, buffAllies: 0.3 }, { name: '西凉士兵', hp: 100, attack: 25 }, { name: '西凉士兵', hp: 100, attack: 25 }]
    ],
    elite: [
      [{ name: '西凉骁将·胡轸', hp: 600, attack: 45, blockGain: 20, summon: true }, { name: '西凉亲兵', hp: 120, attack: 25 }, { name: '西凉亲兵', hp: 120, attack: 25 }],
      [{ name: '飞熊军百夫长', hp: 550, attack: 40, charge: 100 }, { name: '飞熊军锐士', hp: 130, attack: 30, bleed: 1 }, { name: '飞熊军锐士', hp: 130, attack: 30, bleed: 1 }]
    ],
    boss: [[{ name: '骁骑校尉·华雄', hp: 1200, attack: 80, boss: true }, { name: '西凉近卫', hp: 300, attack: 35 }, { name: '西凉近卫', hp: 300, attack: 35 }]]
  },
  {
    normal: [
      [{ name: '曹军盾兵', hp: 150, attack: 25, blockGain: 20 }, { name: '曹军盾兵', hp: 150, attack: 25, blockGain: 20 }, { name: '曹军弓手', hp: 100, attack: 30 }, { name: '曹军弓手', hp: 100, attack: 30 }],
      [{ name: '陷阵营死士', hp: 180, attack: 40, berserkThreshold: 0.3 }, { name: '陷阵营死士', hp: 180, attack: 40, berserkThreshold: 0.3 }, { name: '陷阵营死士', hp: 180, attack: 40, berserkThreshold: 0.3 }],
      [{ name: '曹军谋士', hp: 130, attack: 15, weakenPlayer: true }, { name: '虎贲卫', hp: 160, attack: 35 }, { name: '虎贲卫', hp: 160, attack: 35 }],
      [{ name: '吕布斥候骑', hp: 220, attack: 25, multiAttack: 2 }, { name: '吕布斥候骑', hp: 220, attack: 25, multiAttack: 2 }]
    ],
    elite: [
      [{ name: '曹军校尉·于禁', hp: 750, attack: 55, blockGain: 20 }, { name: '曹军甲士', hp: 180, attack: 30 }, { name: '曹军甲士', hp: 180, attack: 30 }],
      [{ name: '陷阵营副将·高顺部将', hp: 800, attack: 45, summon: true }, { name: '陷阵营死士', hp: 200, attack: 35, deathReflect: 30 }, { name: '陷阵营死士', hp: 200, attack: 35, deathReflect: 30 }]
    ],
    boss: [[{ name: '陷阵营统帅·高顺', hp: 1800, attack: 90, boss: true }, { name: '陷阵营统领', hp: 400, attack: 40 }, { name: '陷阵营统领', hp: 400, attack: 40 }]]
  }
];
function cloneEnemy(enemy) { return { ...enemy, maxHp: enemy.hp, block: 0, burn: 0, berserk: false, stunned: 0, attackBonus: 0 }; }
function nextRandom() {
  if (!state.run) return Math.random();
  const current = Number.isFinite(state.run.rngState) ? state.run.rngState : state.run.seed || 1;
  state.run.rngState = (Math.imul(current, 1664525) + 1013904223) >>> 0;
  return state.run.rngState / 4294967296;
}
function randomIndex(length) { return Math.floor(nextRandom() * length); }
function enemyGroupFor(index, chapter = 0) { const groups = chapterEnemyGroups[chapter] || enemyGroups; const pool = index === 2 ? groups.boss : index === 1 ? groups.elite : groups.normal; return pool[randomIndex(pool.length)].map(cloneEnemy); }
...
function freshBattle(enemyIndex = 0) {
  ...
  state.battle = { turn: 1, chapter: state.chapter || 0, energy: state.run.relics.includes('sunzi') ? 4 : 3, player: { name: hero.name, hp: Math.min(state.run.hp ?? maxHp, maxHp), maxHp, block: 0, rage: 0, rageCap: 10, rageMultiplier: 0.05, mystery: 0, dragon: 0, played: 0, attackCardsPlayed: 0, evade: false, weak: 0, vulnerable: false, bleed: 0 }, enemies, targetIndex: null, enemy: enemies[0], drawPile: shuffle(deck), hand: [], discard: [], logs: [], enemyIndex, relicTriggered: false };
  draw(state.battle, 5);
...
function shuffle(input) { const output = [...input]; for (let i = output.length - 1; i > 0; i--) { const j = randomIndex(i + 1); [output[i], output[j]] = [output[j], output[i]]; } return output; }
...
function enemyAttack(s, enemy = s.enemy) { const eliteBonus = s.enemyIndex === 1 ? Math.floor((s.turn - 1) / 3) * 20 : 0; const bossBonus = s.enemyIndex === 2 && enemy.hp <= enemy.maxHp * 0.3 ? 2 : s.enemyIndex === 2 && enemy.hp <= enemy.maxHp * 0.5 ? 1.2 : 1; const berserkBonus = enemy.berserk ? 2 : 1; return Math.round((enemy.attack + eliteBonus + (enemy.attackBonus || 0)) * bossBonus * berserkBonus); }
...
function enemyStrike(s, enemy, raw, label = `${enemy.name}攻击`) {
  if (s.player.evade) { log('单骑救主抵消了本次攻击。'); s.player.evade = false; return; }
  if (enemy.armorBreak) { const broken = Math.min(s.player.block, enemy.armorBreak); s.player.block -= broken; if (broken) log(`${enemy.name}破除 ${broken} 点护甲。`); }
  const attack = Math.round(raw * (s.player.vulnerable ? 1.1 : 1) * (s.player.weak > 0 ? 0.75 : 1));
  const blocked = Math.min(s.player.block, attack); s.player.block -= blocked; const damage = attack - blocked; s.player.hp -= damage;
  log(`${label}，造成 ${damage} 伤害${blocked ? `（护甲抵消 ${blocked}）` : ''}${s.player.weak > 0 ? '（虚弱）' : ''}。`);
  if (damage > 0 && enemy.bleed) { s.player.bleed += enemy.bleed; log(`${enemy.name}施加 ${enemy.bleed} 层流血。`); }
  if (damage > 0 && s.player.reflectDamage) { const reflected = Math.min(s.player.reflectDamage, enemy.hp); enemy.hp -= reflected; log(`坚壁反戈：反弹 ${reflected} 伤害。`); }
  if (damage > 0 && s.player.reflectRatio) { const reflected = Math.min(Math.round(damage * s.player.reflectRatio), enemy.hp); enemy.hp -= reflected; log(`一夫当关：反弹 ${reflected} 伤害。`); }
  if (state.heroId === 'zhangfei' && damage > 0) { addRage(s, s.player.hp < s.player.maxHp * 0.5 ? 2 : 1); if (s.player.rageOnHit) addRage(s, 1); }
}
function bossSpecialAct(s, enemy) {
  if (!enemy.boss) return false;
  const chapter = s.chapter || 0;
  if (chapter === 0 && s.turn % 3 === 0) { enemyStrike(s, enemy, 70, `${enemy.name}释放黄天符法`); s.player.weak = 2; log('黄天符法：附加 2 回合虚弱。'); return true; }
  if (chapter === 0 && s.turn % 2 === 0) { const helpers = s.enemies.filter(item => item !== enemy && item.hp > 0).length; const armor = helpers ? 20 : 10; enemy.block += armor; log(`${enemy.name}凝聚符法，获得 ${armor} 护甲。`); return true; }
  if (chapter === 1 && s.turn % 2 === 0) { const removed = Math.floor(s.player.block * 0.5); s.player.block -= removed; enemyStrike(s, enemy, 80, `${enemy.name}释放斩击${removed ? `，破除 ${removed} 护甲` : ''}`); return true; }
  if (chapter === 2 && s.turn % 4 === 0 && enemy.hp > enemy.maxHp * 0.3) { const soldiers = [cloneEnemy({ name: '陷阵营死士', hp: 180, attack: 30 }), cloneEnemy({ name: '陷阵营死士', hp: 180, attack: 30 })]; s.enemies.push(...soldiers); log(`${enemy.name}召唤 ${soldiers.length} 名陷阵营死士。`); return true; }
  return false;
}
function healAllies(s, enemy, amount) {
  const healed = s.enemies.filter(target => target.hp > 0 && target.hp < target.maxHp);
  healed.forEach(target => { target.hp = Math.min(target.maxHp, target.hp + amount); });
  if (healed.length) log(`${enemy.name}治疗全体友军 ${amount} 兵力。`);
}
function summonAlly(s, enemy) {
  if (!enemy.summon || s.enemies.length >= 6 || s.turn % 2 !== 0) return false;
  const soldier = cloneEnemy({ name: enemy.summonName || '黄巾亲兵', hp: enemy.summonHp || 80, attack: enemy.summonAttack || 20, bleed: enemy.summonBleed || 0, deathReflect: enemy.summonDeathReflect || 0 });
  s.enemies.push(soldier); log(`${enemy.name}召唤 ${soldier.name}。`); return true;
}
function markEnemyDefeats(s) {
  s.enemies.forEach(enemy => {
    if (enemy.hp > 0 || enemy.defeatHandled) return;
    enemy.defeatHandled = true;
    if (enemy.deathReflect) { s.player.hp -= enemy.deathReflect; log(`${enemy.name}阵亡反伤 ${enemy.deathReflect} 点。`); }
  });
}
function enemyAct(s) {
  if (s.player.bleed > 0) { const damage = s.player.bleed * 8; s.player.hp -= damage; log(`流血：受到 ${damage} 点伤害。`); s.player.bleed--; }
  const alive = s.enemies.filter(enemy => enemy.hp > 0);
  for (const enemy of alive) {
    s.enemy = enemy;
    if (enemy.burn) { enemy.hp = Math.max(0, enemy.hp - enemy.burn * 12); log(`${enemy.name}受到灼烧 ${enemy.burn * 12} 伤害。`); enemy.burn--; }
    if (enemy.stunned > 0) { log(`${enemy.name}被眩晕，无法行动。`); enemy.stunned--; continue; }
    if (enemy.hp <= 0) continue;
    if (bossSpecialAct(s, enemy)) continue;
    if (enemy.healer) { healAllies(s, enemy, 20); continue; }
    if (enemy.buffAllies) {
      s.enemies.filter(target => target !== enemy && target.hp > 0).forEach(target => { target.attackBonus = Math.max(target.attackBonus || 0, Math.round(target.attack * enemy.buffAllies)); });
    ...
    if (s.turn % 2) {
      const attacks = enemy.multiAttack || 1;
      for (let i = 0; i < attacks && s.player.hp > 0; i++) enemyStrike(s, enemy, enemyAttack(s, enemy), attacks > 1 ? `${enemy.name}第 ${i + 1} 次攻击` : undefined);
    } else { enemy.block += enemy.blockGain || 20; log(`${enemy.name}获得 ${enemy.blockGain || 20} 护甲。`); }
  }
  markEnemyDefeats(s);
  if (s.player.weak > 0) s.player.weak--;
  s.player.rageOnHit = false; s.player.reflectDamage = 0; s.player.reflectRatio = 0; s.player.vulnerable = false;
  syncTarget(s);
}
function refreshSelectionVisuals() { document.querySelectorAll('.hand .card').forEach((card, index) => card.classList.toggle('selected', index === state.selectedIndex)); document.querySelector('.hand')?.classList.toggle('has-selection', state.selectedIndex !== null); document.querySelectorAll('[data-target-index]').forEach(target => { const isTargeted = state.selectedIndex !== null && cardNeedsEnemy(cards[state.selectedCard]) && state.battle?.targetIndex !== null && Number(target.dataset.targetIndex) === state.battle?.targetIndex; target.classList.toggle('selected-target', isTargeted); target.classList.toggle('is-target', isTargeted); }); }
function cardNeedsEnemy(card) { return card?.type === 'attack'; }
function cardPreviewView() { if (state.selectedIndex === null || !state.selectedCard) return ''; const card = cards[state.selectedCard]; if (!card) return ''; return `<div class="card-preview-copy"><b>${card.name}</b><span>${card.cost} 气力 · ${cardNeedsEnemy(card) ? '选择敌人' : '立即生效'}</span><small>${card.desc}</small></div>${cardNeedsEnemy(card) ? '<small class="card-preview-hint">点敌人确认</small>' : '<button class="secondary card-use" data-action="use-card">使用</button>'}`; }
function cancelSelection() { if (state.selectedIndex === null) return; state.selectedCard = null; state.selectedIndex = null; state.battle.targetIndex = null; refreshBattleParts(); }
function playCard(index) { const s = state.battle; const id = s.hand[index]; const card = cards[id]; const hasAliveEnemy = s.enemies?.some(enemy => enemy.hp > 0); if (state.busy || !card || s.energy < card.cost || !hasAliveEnemy) return; if (state.selectedIndex === index) return cancelSelection(); s.targetIndex = null; state.selectedCard = id; state.selectedIndex = index; refreshBattleParts(); }
...
function chooseReward(id) {
  ...
  if (rewardType === 'boss-card') { state.reward = { type: 'boss-relic' }; render(); return; }
  finishBattleReward();
}
...
function obtainSoul() { const available = Object.keys(souls).filter(id => !state.run.souls.includes(id)); if (state.run.souls.length >= 2 || !available.length) { state.run.gold += 15; return; } state.run.souls.push(available[randomIndex(available.length)]); }
...
function mapView() { const chapter = state.chapter || 0; const routes = chapterRouteColumns[chapter] || routeColumns; const totalLayers = chapterRoutes[chapter].layers; const options = routes[state.node] || []; const start = Math.max(0, Math.min(state.node - 2, routes.length - 5)); const visible = routes.slice(start, start + 5); const path = (state.run?.path || []).filter(item => item.chapter === chapter); const recentPath = path.slice(-5).map(item => `${item.node + 1}层·${routes[item.node]?.find(option => option.id === item.type)?.label || item.type}`).join('　›　'); return shell(`${topbar()}<section class="map-card"><h2>${chapterRoutes[chapter].name}</h2><p class="map-summary">主将：${heroes[state.heroId].name}　·　章节 ${chapter + 1}/3　·　行军进度 ${Math.min(state.node + 1, totalLayers)}/${totalLayers}　·　主题：${chapterRoutes[chapter].theme}</p><div class="map-line">${visible.map((column,i)=>{const index=start+i;const labels=column.map(option=>`<span class="node-type ${option.id}">${option.mark} ${option.label}</span>`).join('');return `<div class="node ${index<state.node?'done':''} ${index===state.node?'active':''}"><button disabled>${index<state.node?'✓':index===state.node?'⚔':'·'}</button><div class="node-types">${labels}</div></div>`;}).join('')}</div>${recentPath ? `<p class="path-review"><b>最近行军：</b>${recentPath}</p>` : ''}<h3 class="route-heading">选择下一处行军节点</h3><div class="route-options">${options.map(option=>`<button class="route-option" data-node-type="${option.id}"><span class="route-mark">${option.mark}</span><span><b>${option.label}</b><small>${option.desc}</small></span><span>›</span></button>`).join('')}</div></section>`); }
...
function statusChip(label, value, tone = '') { return value > 0 ? `<span class="status-chip ${tone}">${label} ${value}</span>` : ''; }
function enemyStatusView(enemy) { return `<div class="status-row">${statusChip('破甲', enemy.armorBreak, 'danger')}${statusChip('眩晕', enemy.stunned, 'control')}${statusChip('灼烧', enemy.burn, 'danger')}${enemy.berserk ? '<span class="status-chip danger">暴走</span>' : ''}</div>`; }
function enemyPanelView(s) { return `<section id="enemy-panel" class="enemy-group" aria-label="敌方单位">${s.enemies.map((enemy,index)=>`<button class="enemy-bar ${enemy.hp <= 0 ? 'defeated' : ''} ${state.selectedIndex !== null && cardNeedsEnemy(cards[state.selectedCard]) && index === s.targetIndex ? 'selected-target' : ''}" data-target-index="${index}" ${enemy.hp <= 0 ? 'disabled' : ''}><div class="avatar">${s.enemyIndex===2?'将':'敌'}</div><div><div class="enemy-name">${enemy.name}</div><div class="hp-text">兵力 ${enemy.hp}/${enemy.maxHp}</div><div class="bar"><span style="width:${Math.max(0,enemy.hp/enemy.maxHp*100)}%"></span></div>${enemyStatusView(enemy)}</div><div class="intent">${enemy.hp > 0 ? (enemy.boss && s.chapter === 0 && s.turn % 3 === 0 ? '黄天符法 · 70伤害' : enemy.boss && s.chapter === 1 && s.turn % 2 === 0 ? '斩击 · 破甲' : enemy.boss && s.chapter === 2 && s.turn % 4 === 0 && enemy.hp > enemy.maxHp * .3 ? '召唤死士' : s.turn % 2 ? `攻击 ${enemyAttack(s, enemy)}` : `护甲 +${enemy.blockGain || 20}`) : '已击破'}</div></button>`).join('')}</section>`; }
function battleView() { const s=state.battle, p=s.player; const mechanism = state.heroId==='zhangfei' ? `狂怒 ${p.rage}/${p.rageCap || 10}层` : state.heroId==='zhugeliang' ? `神机 ${p.mystery}/${mysteryCap()}层` : `龙胆 ${p.dragon}/3`; return shell(`<div class="battle-screen">${topbar()}${enemyPanelView(s)}<section id="battle-log" class="log-panel"><h2>战斗日志</h2>${s.logs.slice(-8).map(x=>`<div class="log-line">${x}</div>`).join('')}</section><section id="player-panel" class="player-panel"><div class="avatar">${heroes[state.heroId].mark}</div><div><div class="player-heading"><b>${p.name}</b><span>兵力 ${p.hp}/${p.maxHp}</span></div><div class="bar"><span style="width:${Math.max(0,p.hp/p.maxHp*100)}%"></span></div><div class="stats"><span class="stat">护甲 <b>${p.block}</b></span><span class="stat">气力 <b>${s.energy}/3</b></span><span class="stat">${mechanism}</span></div>${activeBonds().length?`<div class="relic-row"><span class="bond-chip">羁绊：${activeBonds().join('、')}</span></div>`:''}${state.run.souls.length?`<div class="relic-row">${soulView()}</div>`:''}${state.run.relics.length?`<div class="relic-row">${relicView()}</div>`:''}</div></section><section id="hand-panel" class="hand-area"><div id="card-preview" class="card-preview ${state.selectedIndex === null ? 'empty' : ''}" role="status">${cardPreviewView()}</div><div class="hand-title"><span>‹ 手牌 ›</span><span>${s.hand.length} 张</span></div><div class="hand">${s.hand.map((id,i)=>cardView(id,i)).join('')}</div><div class="battle-actions"><span class="turn">${state.selectedIndex !== null && cardNeedsEnemy(cards[state.selectedCard]) ? '请选择敌方目标使用' : '点击卡牌查看详情，再点击“使用”'}</span><button class="primary" data-action="end">结束回合</button></div></section></div>${state.reward?rewardModal():''}`); }
function rewardModal() { const isBoss = state.reward.type === 'boss-card'; const isRelic = state.reward.type === 'relic' || state.reward.type === 'boss-relic'; const cardOptions = isBoss ? ['peerless','earthSmash','lineage'] : ['quick','roar','bridge']; if (isRelic) return `<div class="modal-backdrop"><div class="modal"><h2>${state.reward.type === 'boss-relic' ? '章节 Boss 战胜利 · 选择军械' : '精英战胜利 · 选择遗物'}</h2><p style="text-align:center;color:#6b5b42">军械会影响本局后续所有战斗。</p><div class="relic-grid">${Object.entries(relics).map(([id,r])=>`<button class="relic-offer" data-relic="${id}"><span class="route-mark">${r.mark}</span><b>${r.name}</b><small>${r.desc}</small></button>`).join('')}</div></div></div>`; return `<div class="modal-backdrop"><div class="modal"><h2>${isBoss ? '章节 Boss 战胜利 · 选择传说牌' : '战斗胜利 · 选择战利品'}</h2><p style="text-align:center;color:#6b5b42">${isBoss ? '击败章节主将后，从三张张飞传说牌中选择一张。' : '基础牌与张飞普通专属牌组成当前奖励池。'}</p><div class="reward-grid">${cardOptions.map((id,i)=>cardView(id,i,true)).join('')}</div><div class="actions"><button class="secondary" data-action="skip-reward">跳过</button></div></div></div>`; }
function resultView() { const win = state.chapter === chapterRoutes.length - 1 && state.node >= (chapterRouteColumns[2]?.length || 19) - 1; return shell(`${topbar()}<section class="result-card"><h2>${win?'三章凯旋 · 行军完成':'兵败暂退'}</h2><p style="text-align:center;color:#554b3b;line-height:1.8">${win?'你穿越涿郡、虎牢与徐州三大战场，完成了整段行军。':'战场风云未定，带回的残片仍能用于下一次整军。'}</p><div class="stats" style="justify-content:center;margin:20px 0"><span class="stat">本局获得兵革残片 <b>${win?'30':'12'}</b></span><span class="stat">完成章节 <b>${win?'3/3':`${state.chapter + 1}/3`}</b></span><span class="stat">永久总计 <b>${state.meta.shards}</b></span></div><div class="actions"><button class="primary" data-action="start">再次出征</button></div></section>`); }
function bind() {
  ...
  document.querySelector('[data-action="confirm-hero"]')?.addEventListener('click',()=>{const seed=createSeed();state.run={deck:defaultDeck(),gold:80,hp:maxHpForHero(),relics:[],souls:[],upgraded:{},path:[],seed,rngState:seed};state.meta.stats.runs++; saveMeta(); state.screen='map';render();});
  ...
  document.querySelector('[data-action="use-card"]')?.addEventListener('click',confirmTarget);
