...
function freshBattle(enemyIndex = 0) {
  ...
  state.battle = { turn: 1, energy: 3, player: { name: hero.name, hp: state.run.hp ?? hero.maxHp, maxHp: hero.maxHp, block: 0, rage: 0, mystery: 0, played: 0, evade: false }, enemy: { ...enemyData, maxHp: enemyData.hp, block: 0, burn: 0 }, drawPile: shuffle(deck), hand: [], discard: [], logs: [], enemyIndex };
...
function restChoice(choice) {
  if (choice === 'heal') state.run.hp = Math.min(heroes[state.heroId].maxHp, state.run.hp + 70);
...
function render() { app.innerHTML = state.screen === 'title' ? titleView() : state.screen === 'heroes' ? heroView() : state.screen === 'map' ? mapView() : state.screen === 'battle' ? battleView() : state.screen === 'event' ? eventView() : state.screen === 'shop' ? shopView() : state.screen === 'rest' ? restView() : resultView(); bind(); }
...
function bind() {
  ...
  document.querySelector('[data-action="confirm-hero"]')?.addEventListener('click',()=>{state.run={deck:defaultDeck(),gold:80,hp:heroes[state.heroId].maxHp};state.screen='map';render();});
